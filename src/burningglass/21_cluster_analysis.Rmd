...
military_transition_unemployment >= 5 & military_transition_unemployment < 10 ~ "5 - 10",
military_transition_unemployment >= 11 & military_transition_unemployment < 20 ~ "11 - 20",
...
assign(name, clusterpam, envir = .GlobalEnv)
}
...
seq.10cluster_20$stats

cluster_20 <- seq.10cluster_20$clustering
...
# Visualizing 20 years 10 cluster solution --------------------------------------------------
seq.30cluster_10$stats

cluster_20 <- seq.30cluster_10$clustering
cluster_20_fac <- factor(cluster_20)
all <- rownames(seq.10)
all_df <- as.data.frame(cbind(all, cluster_20))
colnames(all_df) <- c("id", "cluster")
...
# Creating sequence objects fo 10, 20, and 30 years after exiting the military and for nonveterans after 10 years in the workforce
...
all_df <- left_join(all_df, covariates, by = "id") %>% mutate(cluster_group = case_when(
  cluster %in% c(27, 476, 1389, 1491, 1505) ~ "education",
cluster %in% c(2874, 4578, 4656, 4963) ~ "promotion",
cluster %in% c(2886, 4647) ~ "demotion",
cluster %in% c(4588, 4613, 4628) ~ "unemployment",
cluster %in% c(2880, 2889, 4388, 4653, 4976, 5111) ~ "no change"
),
cluster_label = case_when(
  cluster == 27 ~ "zone 5 to education to zone 5",
  cluster == 476 ~ "zone 3 to education",
  cluster == 1389 ~ "zone 4 to education to zone 4",
  cluster == 1491 ~ "zone 4 to education",
  cluster == 1505 ~ "education to zone 4",
  cluster == 2874 ~ "zone 4 to zone 5",
  cluster == 2880 ~ "zone 5",
  cluster == 2886 ~ "zone 5 to zone 4",
  cluster == 2889 ~ "zone 4 to zone 5 zone 4",
  cluster == 4388 ~ "zone 4 to zone 3 to zone 4",
  cluster == 4578 ~ "zone 3 to zone 4",
  cluster == 4588 ~ "zone 3 to unemployment to zone 4",
  cluster == 4613 ~ "zone 4 to unemployment to zone 4",
  cluster == 4628 ~ "zone 4 to unemployment",
  cluster == 4647 ~ "zone 4 to zone 3",
  cluster == 4653 ~ "zone 4",
  cluster == 4656 ~ "zone 2 to zone 4",
  cluster == 4963 ~ "zone 2 to zone 3",
  cluster == 4976 ~ "zone 3",
  cluster == 5111 ~ "zone 2"
), cluster_long = paste0(cluster_group, " - ", cluster_label))
all_df <- all_df %>% left_join(highest_degree_from_forprofit, by = "id")
...
all_df <- left_join(all_df, covariates, by = "id") %>% mutate(cluster_group = case_when(
  cluster %in% c(127, 177, 358, 604) ~ "education",
cluster %in% c(1023, 1340) ~ "promotion",
cluster %in% c(966) ~ "demotion",
cluster %in% c(76, 1211, 1656) ~ "unemployment",
cluster %in% c(636, 666, 1232, 1418, 1547, 1860, 1926, 1936, 1944, 1984) ~ "no change"
),
cluster_label = case_when(
  cluster == 76 ~ "unemployment",
  cluster == 127 ~ "zone 4 to education to zone 4",
  cluster == 177 ~ "education to zone 4",
  cluster == 358 ~ "education to zone 5",
  cluster == 604 ~ "education",
  cluster == 636 ~ "zone 5",
  cluster == 666 ~ "zone 5",
  cluster == 966 ~ "zone 5 to zone 4",
  cluster == 1023 ~ "zone 4 to zone 5",
  cluster == 1211 ~ "zone 4 to unemployment",
  cluster == 1232 ~ "zone 4",
  cluster == 1340 ~ "zone 3 to zone 4",
  cluster == 1418 ~ "zone 4",
  cluster == 1547 ~ "zone 3",
  cluster == 1656 ~ "zone 4 to unemployment to zone 4",
  cluster == 1860 ~ "zone 4",
  cluster == 1926 ~ "zone 3",
  cluster == 1936 ~ "zone 3",
  cluster == 1944 ~ "zone 2",
...
seq.30cluster_7$stats

cluster_20 <- seq.30cluster_7$clustering
cluster_20_fac <- factor(cluster_20)
all <- rownames(seq.10)
all_df <- as.data.frame(cbind(all, cluster_20))
colnames(all_df) <- c("id", "cluster")
...
all_df <- left_join(all_df, covariates, by = "id") %>% mutate(cluster_group = case_when(
  cluster %in% c(232) ~ "zone 5",
cluster %in% c(282) ~ "zone 4",
cluster %in% c(302) ~ "education and zone 4",
