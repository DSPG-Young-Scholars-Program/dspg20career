---
title: "02 Data Profiling DC Metropolitan Area"
...
# Table: Ed
In total, there are `r nrow(ed)` entries in ed table. After a preliminary clean up of self reported degree, we grouped `r round((nrow(degree_clean)-267101)/(nrow(degree_clean))* 100, digits = 2) `% of degree types into 5 categories: some high school, high school, bachelor, master, and doctor's degree. 
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
data_profiling(ed)
...
bgt_res_all <- pers%>%
  left_join(job, by = "id")%>%
  left_join(ed, by = "id")%>%
  left_join(cert, by = "id")

data_profiling(bgt_res_all)
```
