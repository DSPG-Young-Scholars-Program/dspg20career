...
bg_all_demographic <- as.data.table(bg_clean_ls$bg_all_demographic)
bg_vet_demographic <- as.data.table(bg_clean_ls$bg_vet_demographic)
bg_all_job <- as.data.table(bg_clean_ls$bg_all_job)
bg_vet_job <- as.data.table(bg_clean_ls$bg_vet_job)


#select a subset of bg_all_
set.seed(1)
...
first_job <- bg_all_job%>%
  ...
  filter(match == T)%>%
  select(id, start_year, days_in_job_market, days_employed, days_unemployed)%>%
