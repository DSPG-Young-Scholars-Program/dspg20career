...
year_entered_civilian_workforce <- bg_job %>% left_join(bg_covariates, by = "id") %>%
  mutate(duration = end_year - start_year + 1) %>% ungroup() %>% filter(onet_job_zone != 55) %>%
...
#military_duration <- bg_job %>% left_join(bg_covariates, by = "id") %>% filter(veteran == "veteran") %>%
#  mutate(duration = end_year - start_year + 1) %>% filter(onet_job_zone == 55) %>%
#  distinct(id, start_year, end_year, duration) %>% group_by(id) %>%
#        mutate(military_duration = sum(duration))

#career_before_military <- bg_job %>% 
...
#    per_year <- bg_job %>% filter(id %in% vet_ids) %>% 
...
stratified <- stratified(sample, c("gender", "highest_degree", "zipcode"), 0.02)
?splitstackshape::stratified

table(stratified$zipcode, stratified$highest_degree)
...
stratified %>% mutate(total = n()) %>% group_by(highest_degree) %>% count() #mutate(percent = round(n()/total * 100, 2)) %>% distinct(highest_degree, percent) %>% arrange(-percent)
...
highest_jobs %>% group_by(highest_degree, veteran) %>% mutate(total = n()) %>% group_by(highest_degree, highest_zone, veteran) %>% mutate(percent = round(n()/total * 100, 2)) %>% distinct(highest_degree, highest_zone, veteran, percent) %>% arrange(highest_degree, -highest_zone) %>% filter(!(is.na(highest_degree))) %>%
...
highest_jobs %>% group_by(irr_bin, veteran) %>% mutate(total = n()) %>% group_by(irr_bin, highest_zone, veteran) %>% mutate(percent = round(n()/total * 100, 2)) %>% distinct(irr_bin, highest_zone, veteran, percent) %>% arrange(irr_bin, -highest_zone) %>% filter(!(is.na(irr_bin))) %>%
...
highest_jobs %>% group_by(year_entered_workforce_bin, veteran) %>% mutate(total = n()) %>% group_by(year_entered_workforce_bin, highest_zone, veteran) %>% mutate(percent = round(n()/total * 100, 2)) %>% distinct(year_entered_workforce_bin, highest_zone, veteran, percent) %>% arrange(year_entered_workforce_bin, -highest_zone) %>% filter(!(is.na(year_entered_workforce_bin))) %>%
...
highest_jobs %>% filter(veteran == "veteran") %>% group_by(military_transition_unemployment_bin) %>% mutate(total = n()) %>% group_by(military_transition_unemployment_bin, highest_zone) %>% mutate(percent = round(n()/total * 100, 2)) %>% distinct(military_transition_unemployment_bin, highest_zone, percent) %>% arrange(military_transition_unemployment_bin, -highest_zone) %>% filter(!(is.na(military_transition_unemployment_bin))) %>%
  ggplot(aes(x = factor(military_transition_unemployment_bin), fill = factor(highest_zone), y = percent)) +
  geom_col() +
  scale_fill_viridis(discrete = TRUE) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
skill_stats <- function(skill, name = "skill") {
  skills <- highest_jobs_all_skills %>% group_by({{ skill }}) %>% mutate(total = n()) %>% group_by({{ skill }}, highest_zone) %>% mutate(percent = round(n()/total * 100, 2)) %>% distinct({{ skill }}, highest_zone, percent) %>% arrange({{ skill }}, -highest_zone) %>% filter(!(is.na({{ skill }}))) %>% mutate(skill = name) %>% rename(if_skill = {{ skill }})
  return(skills)
}
...
skill_stats <- function(skill, name = "skill") {
  skills <- highest_jobs_top_skills %>% group_by({{ skill }}) %>% mutate(total = n()) %>% group_by({{ skill }}, highest_zone) %>% mutate(percent = round(n()/total * 100, 2)) %>% distinct({{ skill }}, highest_zone, percent) %>% arrange({{ skill }}, -highest_zone) %>% filter(!(is.na({{ skill }}))) %>% mutate(skill = name) %>% rename(if_skill = {{ skill }})
  return(skills)
}
information <- skill_stats(information, "information")
supply <- skill_stats(supply, "supply")
...
skill_stats <- function(skill, name = "skill") {
  skills <- highest_jobs_all_majors %>% group_by({{ skill }}) %>% mutate(total = n()) %>% group_by({{ skill }}, highest_zone) %>% mutate(percent = round(n()/total * 100, 2)) %>% distinct({{ skill }}, highest_zone, percent) %>% arrange({{ skill }}, -highest_zone) %>% filter(!(is.na({{ skill }}))) %>% mutate(skill = name) %>% rename(if_skill = {{ skill }})
  return(skills)
}
...
top_majors_highest_jobs %>% filter(if_skill == 1) %>% group_by(highest_zone) %>% mutate(rank = rank(percent, ties.method = "first")) %>% arrange(-highest_zone, -rank) %>% group_by(skill) %>% mutate(order = ifelse(highest_zone == 5, rank, 0), order = max(order)) %>%
  ...
  theme(axis.text.x = element_text(angle = 90),
        legend.position = "bottom", plot.margin = unit(c(1,3,1,1), "lines")) +
  guides(fill = guide_legend(ncol = 6, title = "Career Highest Job State")) +
        #p +
  coord_flip(clip = "off") +
  geom_text(data = labels, aes(label = paste0("N = ", total), y = 100, fill = NA),
            hjust = 0, size = 4) +
  xlab("Major (shorthand)") +
