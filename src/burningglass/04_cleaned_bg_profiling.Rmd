...
round((nrow(job)-nrow(job_cleaned1))/nrow(job) *100, digits = 2)


job_cleaned2 <- job_cleaned1%>%
  mutate(job_duration_day = as.numeric(enddate- startdate), tenure = if_else(job_duration_day >= 365, "tenure", "not tenure"))%>%
  filter(job_duration_day > 0)

round((nrow(job_cleaned1)-nrow(job_cleaned2))/nrow(job_cleaned1) *100, digits = 2)
...
bg_full <- pers_cleaned%>%
 ...
 # left_join(cert_cleaned, by="id")%>%  #one person might have multiple certificates
  left_join(job_cleaned2, by = "id")%>% #one person might have multiple job listed
